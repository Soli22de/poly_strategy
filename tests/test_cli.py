...
class CliTests(unittest.TestCase):
    ...
    def test_maker_fill_sim_command_writes_report(self):
        with tempfile.TemporaryDirectory() as tmp:
            snapshots = Path(tmp) / "snapshots.ndjson"
            gamma = Path(tmp) / "gamma.ndjson"
            out = Path(tmp) / "maker-fill.json"
            snapshot_rows = []
            for ts, asks in [
    ...
    def test_maker_hybrid_sim_command_writes_report(self):
        with tempfile.TemporaryDirectory() as tmp:
            snapshots = Path(tmp) / "snapshots.ndjson"
            gamma = Path(tmp) / "gamma.ndjson"
            out = Path(tmp) / "maker-hybrid.json"
            snapshot_rows = []
            for ts, asks in [
                ("2026-05-10T00:00:00Z", [0.67, 0.67, 0.66]),
                ("2026-05-10T00:01:00Z", [0.66, 0.66, 0.65]),
            ]:
                for index, market_id in enumerate(["a", "b", "c"]):
                    snapshot_rows.append(
                        {
        ...
        self.assertIn("completed=3", stdout.getvalue())
