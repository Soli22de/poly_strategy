...
OUT="${OUT:-data/maker-hedge-sim-current.json}"
...
HORIZON_SECONDS="${HORIZON_SECONDS:-120}"
MAX_CANDIDATES_PER_BATCH="${MAX_CANDIDATES_PER_BATCH:-20}"
TOP="${TOP:-50}"
INCLUDE_YES_NO_PAIRS="${INCLUDE_YES_NO_PAIRS:-1}"
...
if [[ ! -s "$SNAPSHOTS" ]]; then
  echo "maker_hedge_observations=0 reason=missing_snapshots out=$OUT"
  exit 0
fi
